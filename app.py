...
@keyframes gridPulse2{0%,100%{opacity:0.04}50%{opacity:0.1}}
...
.sp-wrap::before{content:'';position:absolute;inset:0;background-image:linear-gradient(rgba(59,130,246,0.05) 1px,transparent 1px),linear-gradient(90deg,rgba(59,130,246,0.05) 1px,transparent 1px);background-size:48px 48px;animation:gridPulse2 4s ease-in-out infinite}
...
<style>
    .stApp { background: linear-gradient(135deg,#0a0e27 0%,#1a1f3a 50%,#0f172a 100%); background-attachment: fixed; }
    ...
    .alert-critical { background: linear-gradient(135deg, rgba(239,68,68,0.2) 0%, rgba(220,38,38,0.3) 100%); padding: 20px; border-radius: 16px; color: #fecaca; font-weight: 700; font-size: 1.1rem; margin: 20px 0; border: 2px solid rgba(239,68,68,0.5); border-left: 6px solid #ef4444; }
    .alert-warning { background: linear-gradient(135deg, rgba(251,146,60,0.2) 0%, rgba(249,115,22,0.3) 100%); padding: 20px; border-radius: 16px; color: #fed7aa; font-weight: 700; font-size: 1.1rem; margin: 20px 0; border: 2px solid rgba(251,146,60,0.5); border-left: 6px solid #fb923c; }
    .alert-success { background: linear-gradient(135deg, rgba(34,197,94,0.2) 0%, rgba(22,163,74,0.3) 100%); padding: 20px; border-radius: 16px; color: #bbf7d0; font-weight: 700; font-size: 1.1rem; margin: 20px 0; border: 2px solid rgba(34,197,94,0.5); border-left: 6px solid #22c55e; }
    .alert-info { background: linear-gradient(135deg, rgba(59,130,246,0.2) 0%, rgba(37,99,235,0.3) 100%); padding: 20px; border-radius: 16px; color: #bfdbfe; font-weight: 700; font-size: 1.1rem; margin: 20px 0; border: 2px solid rgba(59,130,246,0.5); border-left: 6px solid #3b82f6; }
...
ferie_10   = st.sidebar.checkbox("✅ Con 10 giornate di ferie (5 Ancona + 5 altri depositi)", value=False)
...
if len(df_filtered) > 0:
    ...
    with kpi2: st.metric("🚌 Turni/giorno Lu-Ve", f"{turni_luv:,}",
                         help="Media dei turni richiesti in un giorno feriale del periodo selezionato")
...
with tab1:
    if len(df_filtered) == 0:
        ...
        if len(df_copertura_filtered) > 0:
            ...
            fig_cop.add_trace(go.Bar(
                x=cop["giorno"], y=cop["assenze_nominali"], name="Assenze roster (FP/R/PS…)",
                marker_color="rgba(71,85,105,0.85)",
                hovertemplate="<b>Assenze roster</b><br>%{x|%d/%m/%Y}: <b>%{y}</b><extra></extra>"
            ), row=1, col=1)
            fig_cop.add_trace(go.Bar(
                x=cop["giorno"], y=cop["assenze_statistiche"], name="Assenze storiche (malattie…)",
                marker_color="rgba(51,65,85,0.90)",
                hovertemplate="<b>Assenze storiche</b><br>%{x|%d/%m/%Y}: <b>%{y:.0f}</b><extra></extra>"
            ), row=1, col=1)
            buf_colors = ["rgba(34,197,94,0.85)" if g >= 0 else "rgba(220,38,38,0.90)" for g in cop["gap"]]
            fig_cop.add_trace(go.Bar(
                x=cop["giorno"], y=cop["gap"], name="Buffer / Deficit",
                ...
                hovertemplate="<b>%{x|%d/%m/%Y}</b><br>Buffer/Deficit: <b>%{y}</b><extra></extra>"
            ), row=1, col=1)
            fig_cop.add_trace(go.Scatter(
                x=cop["giorno"], y=cop["persone_in_forza"], name="Persone in forza",
                mode="lines", line=dict(color="#94a3b8", width=2, dash="dot"),
                hovertemplate="<b>Persone in forza</b><br>%{x|%d/%m/%Y}: <b>%{y}</b><extra></extra>"
            ), row=1, col=1)

            gap_colors2 = ["rgba(34,197,94,0.85)" if g >= 0 else "rgba(220,38,38,0.90)" for g in cop["gap"]]
            fig_cop.add_trace(go.Bar(
                x=cop["giorno"], y=cop["gap"], name="Gap",
                marker=dict(color=gap_colors2, line=dict(width=0.5, color="rgba(255,255,255,0.1)")),
            ...
            fig_cop.add_hline(y=0, line_dash="solid", line_color="rgba(255,255,255,0.5)", line_width=1.5, row=2, col=1)
            if soglia_gap < 0:
                fig_cop.add_hline(
            ...
            fig_cop.update_layout(
                ...
                plot_bgcolor="rgba(15,23,42,0.85)", paper_bgcolor="rgba(15,23,42,0.0)",
                font=dict(color="#cbd5e1", family="Arial, sans-serif"),
                margin=dict(t=60, b=20, l=10, r=10),
            )
            fig_cop.update_xaxes(tickformat="%d/%m", tickangle=-45,
                                 gridcolor="rgba(96,165,250,0.08)", linecolor="rgba(96,165,250,0.2)")
            fig_cop.update_yaxes(gridcolor="rgba(96,165,250,0.08)", linecolor="rgba(96,165,250,0.2)", zeroline=False)
            ...
            cards = [
                (c1,"#64748b","rgba(100,116,139,0.12)","▪ Turni da coprire","Turni che devono essere garantiti ogni giorno"),
                (c2,"#475569","rgba(71,85,105,0.15)","▪ Assenze roster","FP, R, PS, AP, PADm, NF e tutti i codici assenza"),
                (c3,"#334155","rgba(51,65,85,0.20)","▪ Assenze storiche","Infortuni, malattie, L.104, permessi (medie storiche)"),
                (c4,"#22c55e","rgba(34,197,94,0.15)","🟢 Buffer","Verde = margine libero · Rosso = DEFICIT"),
                (c5,"#94a3b8","rgba(148,163,184,0.05)","— Persone in forza","Linea tratteggiata = organico totale del giorno"),
            ]
            for col, border, bg, title, desc in cards:
                with col:
                    st.markdown(
                        f"<div style='background:{bg};border-left:4px solid {border};border-radius:8px;"
                        f"padding:10px 12px;min-height:72px;'>"
...
with tab2:
    if len(df_filtered) == 0:
        ...
        with st2_a:
            ...
            # ── FIX: totals.marker.color accetta SOLO un singolo colore stringa,
            #         non una lista. Usiamo blu per entrambi i totali e poi
            #         sovrascriviamo il colore del gap finale via shape + annotation.
            fig_wf = go.Figure(go.Waterfall(
                ...
                connector={"line": {"color": "rgba(96,165,250,0.35)", "width": 1.5, "dash": "dot"}},
                increasing={"marker": {"color": "#22c55e", "line": {"color": "#16a34a", "width": 1}}},
                decreasing={"marker": {"color": "#ef4444", "line": {"color": "#dc2626", "width": 1}}},
                # totals: colore unico (stringa) — il gap sarà differenziato via annotation
                totals={"marker": {"color": "#3b82f6", "line": {"color": "rgba(255,255,255,0.25)", "width": 1}}},
            ...
            # Annotazione finale gap (verde/rosso)
            col_annot = "#22c55e" if gap_medio_wf >= 0 else "#ef4444"
            label_annot = f"✅ Buffer: +{gap_medio_wf:.0f}" if gap_medio_wf >= 0 else f"🚨 Deficit: {gap_medio_wf:.0f}"
            fig_wf.add_annotation(
                x="= Gap /\nBuffer",
                y=gap_medio_wf + (12 if gap_medio_wf >= 0 else -12),
                text=f"<b>{label_annot}</b>",
                showarrow=True,
                arrowhead=2,
                arrowcolor=col_annot,
                font=dict(size=12, color=col_annot, family="Arial Black"),
                bgcolor="rgba(15,23,42,0.9)",
                bordercolor=col_annot,
                borderwidth=1.5,
                borderpad=7,
            )
            ...
            fig_wf.update_layout(
                ...
                xaxis=dict(gridcolor="rgba(96,165,250,0.08)", linecolor="rgba(96,165,250,0.2)",
                           tickfont=dict(size=11)),
                yaxis=dict(title="Persone (media/giorno)",
                           gridcolor="rgba(96,165,250,0.1)", linecolor="rgba(96,165,250,0.2)",
                           zeroline=True, zerolinecolor="rgba(255,255,255,0.2)", zerolinewidth=1.5),
            )
            ...
            with wk2:
                st.metric("🏥 Assenze previste/giorno", f"{assenze_medie:.0f}",
                          delta=f"−{assenze_medie/autisti_medio*100:.1f}% organico" if autisti_medio > 0 else "",
                          delta_color="inverse")
            with wk3:
            ...
            for col_t, label_t, colore_t in [
                ...
                fig_trend.add_trace(go.Scatter(
                    ...
                    line=dict(color=colore_t, width=2), marker=dict(size=5),
        ...
        with st2_c:
            st.markdown(
                ...
                "<b>PS</b> · <b>AP</b> · <b>PADm</b> · <b>NF</b>.</p>",
                unsafe_allow_html=True
            )
            try:
                d0 = df_filtered["giorno"].min().date()
                ...
                for c_,l_,col_ in palette_nom:
                    fig_ass.add_trace(go.Bar(x=df_assenze_full["giorno"], y=df_assenze_full[c_],
                        name=l_, marker_color=col_, marker_line=dict(width=1, color="rgba(255,255,255,0.4)"),
...
with tab3:
    ...
    st.markdown("<p style='color:#93c5fd;'>Turni giornalieri per deposito rispettando il tipo di giorno "
                "(Lu-Ve / Sa / Do) e il range di validità <b>dal → al</b>.</p>", unsafe_allow_html=True)

    if not turni_cal_ok or len(df_tc_filtered) == 0:
        st.warning("Nessun record trovato per il periodo selezionato.")
        st.info("**Debug**: controlla che i valori `valid` in `turni` (Lu-Ve/Sa/Do) corrispondano "
                "ai `daytype` in `calendar` e che le date `dal`/`al` rientrino nel periodo.")
    else:
        ...
        if len(df_tc_plot) > 0:
            ...
            if show_totale:
                ...
                fig_tc.add_trace(go.Scatter(
                    x=tot_gg["giorno"], y=tot_gg["turni"], name="Totale",
            ...
            fig_tc.update_layout(
                barmode=bmode, height=550, hovermode="x unified",
                plot_bgcolor="rgba(15,23,42,0.8)", paper_bgcolor="rgba(15,23,42,0.5)",
                font=dict(color="#cbd5e1", family="Arial, sans-serif"),
            ...
            try:
                ...
                if len(df_dep_codici) > 0:
                    ...
                    for tipo in sorted(df_dep_codici["valid"].unique()):
                        ...
                        st.markdown(
                            f"<p style='color:#93c5fd;font-weight:700;font-size:1rem;margin:16px 0 8px;"
                            f"letter-spacing:1px;'>{label_t} "
                            ...
                            for j, codice in enumerate(codici[i:i+8]):
                                with cols[j]:
                                    st.markdown(
                                        ...
                                        f"border:1px solid {colore_dep}55;border-left:3px solid {colore_dep};"
                                        f"border-radius:8px;padding:8px 6px;text-align:center;"
                                        f"font-size:0.85rem;font-weight:700;color:#e2e8f0;"
                                        f"letter-spacing:0.5px;margin-bottom:6px;'>{codice}</div>",
                                        unsafe_allow_html=True
                                    )
                else:
            ...
            fig_pie_tc.update_layout(height=450, showlegend=True,
                paper_bgcolor="rgba(15,23,42,0.5)", legend=dict(font=dict(color="#cbd5e1")),
                margin=dict(l=20,r=20,t=20,b=20))
            st.plotly_chart(fig_pie_tc, use_container_width=True, key="pc_14")

            st.markdown("---")
            st.markdown("#### <i class='fas fa-calendar-week'></i> Turni per Tipo Giorno — Lu-Ve / Sabato / Domenica", unsafe_allow_html=True)
            try:
                ...
                if len(agg_dt) > 0:
                    ...
                    fig_dt.update_layout(
                        ...
                        annotations=[
                            dict(x=cat, y=tot_cat[cat], text=f"<b>{int(tot_cat[cat]):,}</b>",
