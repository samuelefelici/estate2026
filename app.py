...
Conero Analytics - Versione Integrata Completa
===============================================
"""
...
# --------------------------------------------------
# LOGIN SCREEN PREMIUM - CONERO ANALYTICS
# --------------------------------------------------
def check_password():
    """Login screen premium Conero Analytics"""
    ...
    /* Rimuovi padding default Streamlit */
    .block-container {
        padding: 0 !important;
    ...
    @keyframes logoGlow {
        ...
        50%       { filter: drop-shadow(0 0 20px rgba(59,130,246,0.9))
                           ...
                           drop-shadow(0 0 75px rgba(37,99,235,0.28)); }
    }
    @keyframes ripple {
    ...
    @keyframes particleFloat {
        ...
        50%  { transform: translateY(-55px) translateX(-10px) scale(0.9); opacity: 0.7; }
        75%  { transform: translateY(-80px) translateX(20px) scale(1.1); opacity: 0.4; }
        100% { transform: translateY(-110px) translateX(5px) scale(1); opacity: 0; }
    }
    ...
    .ripple-ring:nth-child(2) { animation-delay: 0.85s; border-color: rgba(59,130,246,0.22); }
    .ripple-ring:nth-child(3) { animation-delay: 1.7s;  border-color: rgba(239,68,68,0.18); }
    ...
    /* Alert Streamlit nascosti sul login */
    ...
    <div class="login-page">
        ...
        <div class="particle" style="width:4px;height:4px;background:#ef4444;left:25%;bottom:15%;animation-duration:9s;animation-delay:1.5s;"></div>
        <div class="particle" style="width:2px;height:2px;background:#60a5fa;left:72%;bottom:8%;animation-duration:8s;animation-delay:3s;"></div>
        <div class="particle" style="width:3px;height:3px;background:#22c55e;left:82%;bottom:20%;animation-duration:10s;animation-delay:0.8s;"></div>
        <div class="particle" style="width:2px;height:2px;background:#3b82f6;left:45%;bottom:5%;animation-duration:6s;animation-delay:2.2s;"></div>
        <div class="particle" style="width:4px;height:4px;background:#93c5fd;left:60%;bottom:12%;animation-duration:11s;animation-delay:4s;"></div>
        <div class="particle" style="width:2px;height:2px;background:#ef4444;left:8%;bottom:30%;animation-duration:8.5s;animation-delay:1s;"></div>
        <div class="particle" style="width:3px;height:3px;background:#3b82f6;left:90%;bottom:25%;animation-duration:7.5s;animation-delay:3.5s;"></div>
    </div>

    <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;position:relative;z-index:10;">
    ...
    <div class="login-footer">CONERO ANALYTICS © 2026 &nbsp;·&nbsp; SISTEMA PROTETTO &nbsp;·&nbsp; ACCESSO RISERVATO</div>
    """, unsafe_allow_html=True)
...
# --------------------------------------------------
# CSS DASHBOARD PREMIUM (caricato solo dopo login)
# --------------------------------------------------
st.markdown("""
...
<style>
    ...
    .alert-critical {
        ...
        padding: 20px; border-radius: 16px; color: #fecaca; font-weight: 700;
    ...
    .alert-warning {
        ...
        padding: 20px; border-radius: 16px; color: #fed7aa; font-weight: 700;
        font-size: 1.1rem; margin: 20px 0;
    ...
    .alert-success {
        ...
        padding: 20px; border-radius: 16px; color: #bbf7d0; font-weight: 700;
        font-size: 1.1rem; margin: 20px 0;
    ...
    .alert-info {
        ...
        padding: 20px; border-radius: 16px; color: #bfdbfe; font-weight: 700;
        font-size: 1.1rem; margin: 20px 0;
...
deposito_sel = st.sidebar.multiselect("📍 DEPOSITI", depositi, default=depositi)
...
if len(df_filtered) > 0:
    ...
    with kpi3: st.metric("📊 Disponibili/Giorno", f"{int(disponibilita_media_giorno):,}")
    with kpi4: st.metric("⚖️ Gap Medio", f"{int(gap_medio_giorno):,}", delta=f"{gap_pct_medio:.1f}%", delta_color="normal" if gap_medio_giorno >= 0 else "inverse")
    ...
    with ins1:
        ...
        if len(by_dep) > 0:
            ...
            <div class='insight-card'>
                ...
                <p style='font-size:1.1rem;margin:0;'><b>{worst_dep}</b> gap medio: <b>{worst_gap:.1f}</b></p>
    ...
    with ins2:
        ...
        if len(by_cat) > 0:
            ...
            <div class='insight-card'>
                ...
                <p style='font-size:1.1rem;margin:0;'><b>{worst_cat}</b> gap medio: <b>{worst_cat_gap:.1f}</b></p>
    ...
    if len(df_filtered) > 0:
        ...
        with col_main:
            ...
            fig_timeline.add_trace(go.Scatter(
                ...
                marker=dict(size=8), fill='tozeroy', fillcolor='rgba(239,68,68,0.15)'
            ), row=1, col=1)
            fig_timeline.add_trace(go.Scatter(
                ...
                marker=dict(size=8), fill='tozeroy', fillcolor='rgba(34,197,94,0.15)'
            ), row=1, col=1)
...
<div style='text-align:center;padding:30px;'>
    ...
    <p style='font-size:0.9em;color:#93c5fd;margin-top:10px;'>
        ...
        <i class='fas fa-shield-alt'></i> Conero Analytics
    </p>
