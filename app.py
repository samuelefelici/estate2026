...
def load_turni_calendario() -> pd.DataFrame:
    """
    Espande la tabella turni (con data_da / data_a) in righe giornaliere.
    ⚠️  Adatta i nomi delle colonne al tuo schema se necessario.
    """
    query = """
        SELECT
            gs::date            AS giorno,
            t.deposito,
            SUM(t.turni_richiesti) AS turni
        FROM turni t,
             generate_series(t.data_da, t.data_a, INTERVAL '1 day') AS gs
...
with tab1:
    if len(df_filtered) > 0:
        ...
        with col_main:
            ...
            colors_bar = ['#dc2626' if g < soglia_gap else '#fb923c' if g < 0 else '#22c55e'
                          for g in grouped["gap"]]
            fig_timeline.add_trace(go.Bar(
                x=grouped["giorno"], y=grouped["gap"], name="Gap",
                marker=dict(color=colors_bar, line=dict(width=1, color='rgba(255,255,255,0.2)')),
                showlegend=False
            ), row=2, col=1)
...
with tab5:
    if len(df_filtered) > 0:
        ...
        df_sb = df_filtered.groupby(["deposito","categoria_giorno"])["turni_richiesti"].sum().reset_index()
        if len(df_sb) > 0:
            fig_sun = px.sunburst(df_sb, path=['deposito','categoria_giorno'],
        ...
        st.markdown("---")
        st.markdown("#### <i class='fas fa-th'></i> Matrice Correlazioni", unsafe_allow_html=True)
        ...
        st.dataframe(
            df_filtered[['gap','disponibili_netti','assenze_previste']].describe().T.round(2),
